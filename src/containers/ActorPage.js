...
const ActorPage = props => {
  ...
  //   props.getActorCreditMovies(id);
  // }
  // componentDidUpdate(prevProps, prevState) {
  //   if (prevProps.match.params.id !== props.match.params.id) {
  //     props.getPeopleProfile(props.match.params.id);
  //   }
  // }
  const { profile, movies, getPeopleProfile, getActorCreditMovies } = props;
  const { id } = props.match.params;

  useEffect(() => {
    getPeopleProfile(id);
    getActorCreditMovies(id);
    console.log('actor');
  }, [getPeopleProfile, getActorCreditMovies, id]);
